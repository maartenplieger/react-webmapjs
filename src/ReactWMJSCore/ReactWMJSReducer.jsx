  ...
  switch (action.type) {
    ...
    case WEBMAPJS_LAYER_CHANGE_DIMENSION:
      ...
      return produce(state, draft => {
        ...
        const layerIndex = getLayerIndexFromAction(action, mapPanel.layers);
        if (layerIndex === null) return state;
        const dimensions = mapPanel.layers[layerIndex].dimensions || [];
        const dimensionIndex = getDimensionIndexFromAction(action, dimensions);
        ...
        if (mapDimension) {
          ...
          const dimensionIndex = getDimensionIndexFromAction(action, reduxMapDimensions);
          if (dimensionIndex === null) {
    ...
    case WEBMAPJS_MAP_CHANGE_DIMENSION:
      return produce(state, draft => {
        ...
        for (let d = 0; d < wmjsLayers.length; d++) {
          ...
          if (layerDimension) {
            const layerIndex = getLayerIndexFromAction({ payload: { layerId: layer.id } }, mapPanel.layers);
